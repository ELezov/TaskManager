...
- **Карточки задач**: название, приоритет (Низкий, Средний, Высокий, Критикал), срок исполнения, счётчик заметок.
- **Перетаскивание** карточек между колонками (drag-and-drop).
- Клик по карточке открывает **модальное окно** редактирования: название, описание, срок, приоритет, статус и список заметок к задаче (добавление/удаление).
