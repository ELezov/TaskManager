...
- Задачи и заметки привязаны к **сферам** (вкладки: по умолчанию «Работа» и «Жизнь», можно добавлять свои). У каждой сферы уникальный текстовый идентификатор: у начальных — `work` и `life`, у новых — slug от названия при создании.
...
Данные по умолчанию хранятся в браузере (`localStorage`). Кнопка **«Выгрузить JSON»** скачивает файл со сферами и задачами. **«Загрузить JSON»** не заменяет данные целиком: добавляются только те сферы и задачи, которых ещё нет (по уникальному `id`). Если у загружаемой сферы или задачи такой же `id`, как у существующей, появится запрос: сохранить текущую или заменить загруженной. У каждой задачи при создании присваивается уникальный идентификатор.
